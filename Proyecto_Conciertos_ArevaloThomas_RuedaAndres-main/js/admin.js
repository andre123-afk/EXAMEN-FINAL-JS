...
formCategoria.addEventListener("submit", e => {
    ...
    alert("Categoría creada.");function mostrarUltimosPagos() {
        ...
        const ultimos = ventas.slice(0, 3);
    
        ultimos.forEach(venta => {
    
            const nombres = venta.productos.map(p => p.nombre).join(", ");
    
            const cantidad = venta.productos.reduce((total, producto) => {
